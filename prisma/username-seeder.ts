...
async function seedDoctorUsernames() {
  try {
    ...
    for (const doctor of doctors) {
      let baseUsername = generateUsername(doctor.firstName, doctor.lastName);
      ...
      console.log(`Dr ${doctor.firstName} ${doctor.lastName} -> ${finalUsername}`);
    }
    ...
    console.log(`✅ Successfully updated ${updates.length} doctors with usernames`);
